...
class LoopFrame(wx.Frame):
    ...
    def __init__(self, parent, config):
        ...
        self.num_points = int(float(self.pulse_length/config.getVal('awg_ns_per_point')))
    ...
    def init_plot(self):       
        ...
        time_axis = np.arange(0, self.num_points*self.config.getVal('awg_ns_per_point'), self.config.getVal('awg_ns_per_point'))
    ...
    def check_proceed(self): 
        if self.auto_loop == True:
            ...
            prog = wx.ProgressDialog("Ready to write to AWG", "Writing in %i seconds" % self.auto_loop_wait, 
                        self.auto_loop_wait, style=wx.PD_AUTO_HIDE|wx.PD_CAN_ABORT, parent=self.parent)
            while (self.auto_loop_wait - i >= 0):
                if (prog.Update(i, "Writing in %i seconds" % (self.auto_loop_wait-i))[0]) == False:
                    # User pressed cancel
                    proceed = CODES.Abort
                    prog.Destroy()
                    break
    ...
    def run_loop(self):   
        ...
        while (not self.auto_loop) or (self.i<self.iterations and self.rms_error()>=self.tolerance):                      
    ...
    def calculate_parms_for_loop(self):    
        ...
        if self.i == 0:
            ...
            awg_next[np.logical_and(self.target!=0,self.current_output<=threshold)]+=self.awg_zero_shift 
        ...
        try: 
            ...
            def no_filter(data): return data
    ...
    def apply_correction(self):
        if self.sim == True:
            ...
            print(get_message_time()+"Applied correction for iteration %i" % (self.i+1))
        else:
    ...
    def import_awg_filter(self):
        ...
        try:    
            ...
            self.show_error("Can't import module {0}. Ensure {1} is in one of the following locations:\n\n{2}"
            .format(modname, filename, sys.path), "File not found")            
