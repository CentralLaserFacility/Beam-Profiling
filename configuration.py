...
class Configuration(wx.Dialog):
    ...
    def __init__(self, *args, **kwds):
        ...
        self.parms = {}
        self.parms["diag"] = Param(
            label="Diagnostic files location",
            widget=wx.TextCtrl(self),
            section="file_locations",
        )
        self.parms["curve"] = Param(
            label="Library files location",
            widget=wx.TextCtrl(self),
            section="file_locations",
        )
        self.parms["filter"] = Param(
            label="Filter file", widget=wx.TextCtrl(self), section="file_locations"
        )
        self.parms["algorithm"] = Param(
            label="Algorithm file", widget=wx.TextCtrl(self), section="file_locations"
        )
        self.parms["pulse_peak_power"] = Param(
            kind="float", label="Peak Power", widget=wx.TextCtrl(self), section="safety"
        )
        self.parms["auto_loop"] = Param(
            kind="bool", label="Auto loop", widget=wx.CheckBox(self), section="safety"
        )
        self.parms["auto_loop_wait"] = Param(
            kind="int",
            label="Auto loop wait (s)",
            widget=wx.TextCtrl(self),
            section="timing",
        )
        self.parms["scope_wait"] = Param(
            kind="float",
            label="Scope read wait (s)",
            widget=wx.TextCtrl(self),
            section="timing",
        )
        self.parms["awg_wait"] = Param(
            kind="float",
            label="AWG write wait (s)",
            widget=wx.TextCtrl(self),
            section="timing",
        )
        self.parms["scope"] = Param(
            label="Default scope PV", widget=wx.TextCtrl(self), section="pvs"
        )
        self.parms["awg_prefix"] = Param(
            label="AWG_PV prefix", widget=wx.TextCtrl(self), section="pvs"
        )
        self.parms["sim"] = Param(
            kind="bool", label="Simulation", widget=wx.CheckBox(self), section="sim"
        )
        self.parms["awg_zero_shift"] = Param(
            kind="float",
            label="AWG zero shift",
            widget=wx.TextCtrl(self),
            section="awg",
        )
        self.parms["noise_threshold_percentage"] = Param(
            kind="float",
            label="Noise threshold (%)",
            widget=wx.TextCtrl(self),
            section="awg",
        )
        self.parms["awg_ns_per_point"] = Param(
            kind="float",
            label="AWG calib (ns/point)",
            widget=wx.TextCtrl(self),
            section="awg",
        )
        self.parms["awg_write_method"] = Param(
            label="AWG write method",
            widget=wx.ComboBox(self, choices=["pts", "wfm"], style=wx.CB_READONLY),
            section="awg",
        )
        self.parms["epics_ca_addr_list"] = Param(
            label="Channel Access addr list", widget=wx.TextCtrl(self), section="epics"
        )
        self.parms["epics_ca_auto_addr_list"] = Param(
            label="Channel Access auto addr list",
            widget=wx.ComboBox(self, choices=["No", "Yes"], style=wx.CB_READONLY),
            section="epics",
        )
    ...
    def readConfig(self, filename="./config.ini"):
        ...
        for name, parm in self.parms.items():
            ...
            if parm.kind == "bool":
                ...
                parm.value = config.getint(parm.section, name)
                parm.widget.SetValue(str(parm.value))
            else:
    ...
    def epics_setup(self):
        ...
        os.environ["EPICS_CA_AUTO_ADDR_LIST"] = self.parms[
    ...
    def __do_layout(self):
        ...
        for parm in self.parms.values():
            ...
            if parm.widget:
                ...
                self.szr.Add(
                    label, pos=(i, 0), span=(1, 1), flag=wx.EXPAND | wx.ALL, border=5
                )
                self.szr.Add(
                    parm.widget,
                    pos=(i, 1),
        ...
        self.szr.Add(
            self.buttonSizer, pos=(i, width), flag=wx.EXPAND | wx.ALL, border=5
        )
